"""email.py - EmailStrategy: prompt building, parsing, and validation for email sequences."""
...
from services.writing.types import (
    GenerationContext, ValidationResult, WORD_LIMITS, _CATEGORY_DENYLISTS,
)
...
class EmailStrategy:
    ...
    No LLM client, no I/O beyond loading prompt template files.
    ...
    def build_system_prompt(self, ctx: GenerationContext) -> str:
        ...
        sections = load_prompt_sections(
            ...
            seller_product_category=ctx.product_context,
            use_selector=ctx.pea_selector_enabled,
        )
        ...
        return self._build_system_prompt_inline(ctx)

    def _build_system_prompt_inline(self, ctx: GenerationContext) -> str:
...
[body]"""
        ...
        if ctx.opportunity_score is not None and ctx.opportunity_score < 50:
...
The research score for this prospect is {ctx.opportunity_score}/100. The data is thin, ambiguous, or unconfirmed.
...
Use the partial-signal examples as your primary models for this sequence.""")
    ...
    def parse_output(self, raw: str, ctx: GenerationContext) -> tuple[list[dict], list[str]]:
        ...
        # "Subject N: text"
        subject_matches = re.findall(r"[Ss]ubject\s*\d\s*:\s*(.+)", raw)
        if subject_matches:
        ...
        if not emails:
            for i in range(1, 4):
                m = re.search(rf"<email{i}>(.*?)</email{i}>", raw, re.DOTALL)
                if m:
        ...
        if not emails:
            logger.warning("Failed to parse emails from model response. First 500 chars: %s", raw[:500])
    ...
    def validate(self, output: tuple[list[dict], list[str]], ctx: GenerationContext) -> ValidationResult:
        ...
        shorten pipeline after validation.
        """
        emails, subject_options = output

        if len(emails) != 3:
            return ValidationResult(is_valid=False, error_reason=f"Expected 3 emails, got {len(emails)}")

        expected_numbers = {1, 2, 3}
        actual_numbers = {e["email_number"] for e in emails}
        if actual_numbers != expected_numbers:
            return ValidationResult(is_valid=False, error_reason=f"Email numbers must be 1,2,3 — got {sorted(actual_numbers)}")

        for email in emails:
            if not email.get("subject", "").strip():
                return ValidationResult(is_valid=False, error_reason=f"Email {email['email_number']} has empty subject")
            if not email.get("body", "").strip():
                return ValidationResult(is_valid=False, error_reason=f"Email {email['email_number']} has empty body")

        if not subject_options:
            return ValidationResult(is_valid=False, error_reason="No subject options provided")

        return ValidationResult(is_valid=True)

    # ------------------------------------------------------------------
    # Repair / shorten prompts
    # ------------------------------------------------------------------

    def repair_prompt(self, raw: str, ctx: GenerationContext, reason: str) -> str:
        """Return a prompt to repair malformed sequence output."""
        return (
    ...
    def shorten_prompt(self, email: dict) -> str:
        """Return a prompt to rewrite an over-limit email shorter."""
        return (
    ...
    def over_limit_emails(self, emails: list[dict]) -> list[dict]:
    ...
    def limits(self) -> dict:
        """Return word limits per email number."""
        return dict(WORD_LIMITS)
