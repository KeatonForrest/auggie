...
async def _action_push_smartlead(rule, user_id, config, accounts):
    ...
    logger.info("Automation rule %s: pushed %s leads to Smartlead", rule["id"], result.get("pushed", 0))


async def _action_push_outreach(rule, user_id, accounts):
    ...
    for a in accounts:
        if a.get("document_id"):
            draft = await get_outreach_draft(a["document_id"])
            if draft and draft.get("content"):
                ...
                if emails:
                    drafts_by_account[a["id"]] = emails

    if not drafts_by_account:
        raise RuntimeError("No written sequences found for matched accounts")

    result = await push_sequences_to_outreach(user_id, accounts, drafts_by_account)
    logger.info("Automation rule %s: created %s sequences in Outreach", rule["id"], result.get("sequences_created", 0))


async def _action_push_salesloft(rule, user_id, accounts):
