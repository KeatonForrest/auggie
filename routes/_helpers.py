...
def normalize_verticals(stored: list[str]) -> list[str]:
    ...
    out: list[str] = []
    for label in stored:
        mapped = label if label in valid_set else _LEGACY_VERTICAL_MAP.get(label, label)
