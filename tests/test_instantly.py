...
class TestCreateCampaign:
    ...
    async def test_create_campaign_success(self):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = campaign_response
        mock_response.raise_for_status = MagicMock()
        ...
        with patch("services.instantly._get_api_key", new_callable=AsyncMock) as mock_get_key:
            mock_get_key.return_value = "test_api_key"
            with patch("httpx.AsyncClient", return_value=mock_client):
                result = await instantly.create_campaign(1, "My New Campaign")

        assert result == campaign_response
        mock_client.post.assert_called_once_with(
            "https://api.instantly.ai/api/v1/campaign/create",
            json={"api_key": "test_api_key", "name": "My New Campaign"},
        )
        mock_response.raise_for_status.assert_called_once()

    @pytest.mark.asyncio
    async def test_create_campaign_with_special_characters(self):
        """Test creating campaign with special characters in name."""
        campaign_name = "Campaign: Test & Demo (2024)"
        campaign_response = {"id": "campaign_456", "name": campaign_name}
        ...
        with patch("services.instantly._get_api_key", new_callable=AsyncMock) as mock_get_key:
            mock_get_key.return_value = "test_api_key"
            with patch("httpx.AsyncClient", return_value=mock_client):
                result = await instantly.create_campaign(1, campaign_name)

        assert result == campaign_response
        call_args = mock_client.post.call_args
        assert call_args[1]["json"]["name"] == campaign_name

    @pytest.mark.asyncio
    async def test_create_campaign_http_error(self):
        """Test create_campaign raises HTTPError on bad response."""
        from httpx import HTTPStatusError
        ...
        with patch("services.instantly._get_api_key", new_callable=AsyncMock) as mock_get_key:
            ...
            with patch("httpx.AsyncClient", return_value=mock_client):
                with pytest.raises(HTTPStatusError):
                    await instantly.create_campaign(1, "Test Campaign")
